<template>
  <button
    id="au-wave-btn"
    class="au-btn is-line-1 au-fix-ios-appearance"
    :class="[
      'au-size-' + size,
  ...
  >
    ...
    <view
      v-if="ripple"
      class="au-wave-ripple"
      :class="[waveActive ? 'au-wave-active' : '']"
      :style="{
        top: rippleTop + 'px',
        left: rippleLeft + 'px',
  ...
  methods: {
    ...
    getElQuery() {
      return new Promise(resolve => {
        ...
        //#endif
        queryInfo.select('.au-btn').boundingClientRect()
        queryInfo.exec(data => {
          resolve(data)
        })
      })
