 ...
 * @return {Object}
 **/
function pagination(totalData = [], pageNo = 1, pageSize = 10) {
  ...
  if (pageSize >= length) {
    ...
    tableData.pageNo = 1 //直接取第一页
  } else {
    //pageSize小于总数据长度，数据多余1页
    const num = pageSize * (pageNo - 1) //计算当前页（不含）之前的所有数据总条数
    if (num < length) {
